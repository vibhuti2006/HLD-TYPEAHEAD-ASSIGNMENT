...
![Suggestions dropdown for the prefix "macb", ranked by popularity](screenshots/macb_search.png)
*Live suggestions: typing `macb` returns the top 10 matching queries, ranked by all-time count (Popularity mode).*
...
![/cache/debug output showing the owning node and per-node key counts](screenshots/macb_cache_debug.png)
*Consistent hashing in action: the prefix `macb` is routed to `cache-node-0`, and keys are spread evenly across the three nodes (32 / 33 / 43).*
...
**Demonstration — same prefix, two ranking modes:**

![Popularity mode: macbeth quotes sits at rank 5](screenshots/macb_search.png)
*Popularity: "macbeth quotes" (count 12) sits at rank 5, below "macbeth" (count 31).*

![Trending mode: macbeth quotes jumps to rank 1](screenshots/macb_trending.png)
*Trending: after searching "macbeth quotes" several times, it jumps to **rank 1 — above "macbeth"**, even though its all-time count (12) is far lower. A lower-count query outranking a higher-count one can only happen via the recency boost, which proves recency-aware ranking works.*
...
Full measured results are in **[PERFORMANCE_REPORT.md](PERFORMANCE_REPORT.md)**,
generated by `loadtest.py`. To reproduce:

```bash
# terminal 1 — start the server
.venv/bin/uvicorn app.main:app --port 8099
# terminal 2 — fire load and write PERFORMANCE_REPORT.md
.venv/bin/python loadtest.py
```

`loadtest.py` uses only the standard library, fires thousands of requests under
concurrency, and reports numbers as **deltas of the server's `/stats`**, so they
are real measurements (not hand-picked). Representative results:

| Metric | Value | Meaning |
|--------|-------|---------|
| `/suggest` p50 / p95 latency | ~5 ms / ~8 ms (client, 20 concurrent) | server-side handler time is ~0.005 ms; the rest is HTTP overhead |
| cache hit rate | **~98%** over 5000 requests | repeated prefixes hit the cache |
| write reduction | **~98%** (2000 searches → ~40 DB writes) | batching + aggregation |
| consistent hashing | 318 keys → ~31% / 38% / 31% over 3 nodes | even key distribution |

![/stats endpoint after a load-test run](screenshots/stats.png)
*Live `/stats` after a load test: 97.9% cache hit rate, sub-millisecond server-side latency, and 2000 searches collapsed into 40 DB write transactions (`write_reduction_ratio` 0.98).*
...
```
ingest.py              # raw AOL log  ->  data/queries.csv  (aggregate + top-N + content filter)
loadtest.py            # fires load, writes PERFORMANCE_REPORT.md
PERFORMANCE_REPORT.md  # measured latency / hit rate / write reduction
